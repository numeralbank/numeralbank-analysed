...
fn <- paste0("Figures/worldmap_", variable_to_be_plotted, ".png")
...
fillScale <- scale_fill_manual(name = "the.bases",values = myColors)
  ...
  ggsave(fn,  width =30, height = 20, units = "cm", dpi = 900)

#histogram
the.nas <- sum(is.na(languages$BestBase))
#freq.bestbases <- freq.bestbases[c(1:6,8:14,7),]
...
# variable_to_be_plotted_order_vec[15] <- "NA"
...
fn <- paste0("Figures/worldmap_", variable_to_be_plotted, "_legends.png")
